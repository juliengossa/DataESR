...
pkiesr_read.adm <- function() {
    ...
    adm <- adm %>%
      ...
      summarise(
        pki.ADM.P.formations     = n(),
        pki.ADM.S.sélective      = sum(sélective == TRUE, na.rm = TRUE),
        pki.ADM.S.hypersélective = sum(hypersélective == TRUE, na.rm = TRUE),
        pki.ADM.S.surchargées    = sum(surchargée == TRUE),
        pki.ADM.S.souschargée    = sum(souschargée == TRUE),
