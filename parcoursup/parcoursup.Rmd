...
ps.all <- bind_rows(ps2018.all, ps2019.all) %>%
  mutate(
    Rentrée = factor(Rentrée, levels=c("2019","2018")),
    Taux.de.tension =
...
rentrée <- 2019    
ps <- ps.all %>% filter(Rentrée == 2019, Filière.de.formation.très.agrégée != "Autre formation")
...
ps.all %>% filter(Rentrée == rentrée) %>%
  ggplot(aes(x=Capacité.de.l.établissement.par.formation, fill=Filière.de.formation.très.agrégée)) +
...
ggplot(ps, aes(y=Taux.de.tension, fill=Filière.de.formation.très.agrégée)) +
  geom_boxplot() +
  facet_grid(~Filière.de.formation.très.agrégée) +
  ylab("Taux de tension") + scale_fill_discrete(name="Fillère") +
  ggtitle("Taux de tension par filière") +
  guides(fill=FALSE) +
...
ggplot(ps, aes(x=Taux.de.tension, 
...
ggplot(ps, aes(x=Taux.de.remplissage,
                   fill=Filière.de.formation.très.agrégée)) +
...
```{r Taux.de.remplissage.ALL.allemand, echo=FALSE}
...
ggplot(aes(x=Taux.de.remplissage,
           fill=Allemand)) + 
  geom_histogram(bins = 10,color="white",fill="#00BFC4") +
...
ggplot(ps, aes(y=Taux.de.sélection,
                   fill=Filière.de.formation.très.agrégée)) +
  geom_boxplot() +
...
ggplot(subset(ps, Type.de.sélection != "Non_sélective"), 
...
ggplot(ps,aes(x=Filière.de.formation.très.agrégée, fill=Type.de.sélection)) +
  geom_bar(position="fill", colour="black") +
  xlab("Filière") + ylab("") +
  scale_y_continuous(labels=scales::percent) +
...
ggplot(ps.sel.cand, aes(x=1,y=Admis/1000,fill=Type.de.sélection)) +
  geom_bar(stat="identity", colour="black") +
    xlab("") + ylab("") +
...
scales::percent(weighted.mean(ps$Taux.de.sélection,ps$Effectif.total.des.candidats.ayant.accepté.la.proposition.de.l.établissement..admis.,na.rm = TRUE))
```
...
ggplot(ps, aes(x=X..d.admis.néo.bacheliers.boursiers,
                   fill=Filière.de.formation.très.agrégée)) +
...
```{r Taux.de.bac.alluvial, echo=FALSE}
ps %>%
  group_by(Filière.de.formation.très.agrégée) %>%
  ...
  mutate(
    ...
    Filière = fct_relevel(Filière,c("Autre formation","CPGE", "PACES", "Licence","DUT","BTS")),
    Effectif = Effectif/1000) %>%
  
  ggplot(aes(y = Effectif, axis1 = Bac, axis2 = Filière)) +
    geom_alluvium(aes(fill = Bac), width = 1/12) +
    geom_stratum(width = 1/12, fill = "black", color = "grey") +
    geom_label(stat = "stratum", label.strata = TRUE) +
    
    ggtitle("Répartition des effectifs néo-bacheliers\nadmis dans le supérieur (milliers)") +
  theme(axis.text.x = element_blank(), axis.title.x=element_blank(), axis.ticks.x = element_blank()) +
  guides(fill=FALSE) +
  theme_hc()
```
