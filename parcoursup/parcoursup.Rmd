...
ps2018.etab.sel <- ps2018 %>% 
  ...
  pivot_wider(names_from=Type.de.sélection, 
              values_from=c(Taux.de.sélection.moyen, Nb.de.Licences),
              values_fill=list(Nb.de.Licences=0)) %>%
  ...
  ggplot(ps2018.etab.sel, aes(x=Taux.de.Licences.sélectives)) + 
    ...
    xlab("Taux de Licences sélectives") + ylab("Nombre d'établissements") +
  scale_x_continuous(labels=scales::percent) +
...
```{r Taux.de.sélection.établissement.top10, echo=FALSE}
  ...
  rename(
    Nb.L = Nb.Licences,
    Nb.L.sél = Nb.de.Licences_Sélective,
    Taux.L.sél = Taux.de.Licences.sélectives,
    Taux.sél.moyen = Taux.de.sélection.moyen_Sélective) %>%
  arrange(-Nb.L.sél) %>%
  head(10) %>%
  mutate(
    Taux.L.sél = scales::percent(Taux.L.sél),
    Taux.sél.moyen = scales::percent(Taux.sél.moyen)) %>%
  kable() %>%
  kable_styling()
```


```{r Taux.de.sélection.sélection.moyenne.top10, echo=FALSE}
  ps2018.etab.sel %>%
  rename(
    Nb.L = Nb.Licences,
    Nb.L.sél = Nb.de.Licences_Sélective,
