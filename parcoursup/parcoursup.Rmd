...
```{r Taux.de.remplissage.ALL, echo=FALSE}

ps2018 %>%
  filter(Filière.de.formation == "Licence - Arts-lettres-langues") %>%
ggplot(aes(y=Taux.de.remplissage, 
  ...
  scale_y_continuous(labels=scales::percent) +
  theme_hc() +
  ggtitle("Distribution des taux de remplissage des filières de Licence ALL\n(limités à 170%)") +
...
```{r Taux.de.remplissage.DEG, echo=FALSE}

ps2018 %>%
  filter(Filière.de.formation == "Licence - Droit-économie-gestion") %>%
ggplot(aes(y=Taux.de.remplissage, 
           x=reorder(Filière.de.formation.détaillée,
                     Taux.de.remplissage, FUN=median),
           fill=Filière.de.formation.détaillée)) + 
