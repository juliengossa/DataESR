...
```{r pki.raw}
  pkiesr_plot_norm(rentrée,uai,pkiesr_lfc[["K"]], norm.values=FALSE, omit.first = TRUE)
```
...
plot_grid(nrow=1, align = "hv",
   plotlist = pkiesr_plot_evol_all(rentrée, uai, peg.args,
...
```{r etu.raw}
pkiesr_plot_primaire(rentrée,uai,pkiesr_lfc[["ETU"]])
```
...
```{r etu.norm}
pkiesr_plot_norm(rentrée,uai,pkiesr_lfc[["ETU"]])
```
