...
```{r function}
emplois_plot <- function(df, y_breaks, title, fill_guide=FALSE, palette=NULL, palette_limits = c(0,1)) {
  p <- ggplot(df %>% filter(Rentrée != 2020) , aes(x=X, y=Y, fill=FILL)) +
    geom_col(data = df %>% filter(Rentrée==rentrée)) +
    geom_point(aes(shape = Rentrée)) +
    scale_y_continuous(breaks = y_breaks, labels = abs) +
...
emplois.cnu %>%
  ...
  mutate(
    ...
    Y = Emplois,
    FILL = Corps
  ) %>% emplois_plot(seq(-300,600,100), paste("Nombre d'emplois aux sessions synchronisées",rentrée), fill_guide=guide_legend(reverse = TRUE))
```
...
emplois.cnu %>%
  ...
  mutate(
    ...
    Y = Emplois,
    FILL = Corps
  ) %>% emplois_plot(seq(-200,200,50), paste("Nombre d'emplois aux sessions synchronisées",rentrée), fill_guide=guide_legend(reverse = TRUE))
```
...
emplois.cnu %>%
  ...
  mutate(
    ...
    Y = Emplois,
    FILL = Corps
  ) %>% emplois_plot(seq(-40,90,10), paste("Nombre d'emplois aux sessions synchronisées",rentrée), fill_guide=guide_legend(reverse = TRUE))
```
